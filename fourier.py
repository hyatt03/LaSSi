...
import numpy as np
from utils import dot, downsample
import cmath
...
def run_computation_on_q(arg):
    ...
    for particle in particles:
        ...
        for z in range(0, 3):
            # Sum over scattering vector dotted the particles lattice position
            sum_A[z] += position_t[z][0] * q_dot_lattice

            # Prepare data for fourier transform
            fft_data = [x * q_dot_lattice for x in position_t[z]]
...
def parallel_compute_scattering_intensity(options, timeseries, particles):
    ...
    for res in mapping_result:
        I_aa_temp.append(res[0])
        energies.append(res[1])
        frequencies.append(res[2])

    return qs, I_aa_temp, energies, frequencies
...
def plot_fourier(o, filename, total_fourier, f, energy, smoothing = False):
    ...
    plt.savefig(filename.replace('q0', 'q0_0.6'), bbox_inches='tight', dpi=300)

    fig, ax = plt.subplots()
    ax.plot(energy, -y_data)
    plt.xlim(0.09, 0.15, 'bd')
    plt.xlabel('Energy [meV]')
    plt.ylabel('Intensity [A.U.]')

    plt.savefig(filename.replace('q0', 'q0_0.1'), bbox_inches='tight', dpi=300)


# Create a heatmap over intensities and energies for a range of scattering vectors
def plot_energy_spectrum(o, filename, qs, I_aa_temps, energies, frequencies):
    y_data = (I_aa_temps[0][0:len(energies)] + I_aa_temps[1][0:len(energies)] + I_aa_temps[2][0:len(energies)]) / 3
    plt.hist2d(energies, y_data)
    plt.colorbar()
    plt.savefig(filename, bbox_inches='tight', dpi=300)
